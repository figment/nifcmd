...
static bool CullNodes(stringlist expr, NiNodeRef block)
{
   bool ok = false;
   ...
   if (node)
   {
      ...
      for (vector<NiAVObjectRef>::iterator itr = links.begin(), end = links.end(); itr != end; )
      {
         ...
         if (nodeObj) 
         {
            ...
            {
               nodeObj->ClearControllers();
               nodeObj->ClearChildren();
               itr = links.erase(itr);
               ok = true;
               continue;
            }
...
static bool CullNodes(stringlist expr, vector<NiObjectRef>& blocks)
{
   NiObjectRef block = blocks[0];
   NiNodeRef nodeObj = DynamicCast<NiNode>(block);
   return (nodeObj) ? CullNodes(expr, nodeObj) : false;
}
   ...
   case NifCmd::htLong:  
      {
         ...
         cout << "Usage: " << exeName << " CullNodes <input> [-opts[modifiers]]" << endl 
...
REGISTER_COMMAND(CullNodes, HelpString, ExecuteCmd);